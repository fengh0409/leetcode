...
func LengthOfLongestSubstring(s string) int {
	...
	for k, v := range []byte(s) {
		if k0, ok := m[v]; ok {
			...
			// !!!注意：这里一定要判断 k0 >= left
			// 考虑字符串bcaabd，当遍历到第二个a时，由于a重复了，所以滑动窗口缩小窗口，bca被移出了滑动窗口，此时仅包含字符a，窗口的左右边界都是第二个a的下标，即left=right=3，此时m=map[byte]int{'b':0,'c':1,'a':3}。当遍历到第二个b时，若没有k0+1>left的判断，则left=0+1=1，窗口的左边界往左边移动了！！此时子串长度为right-left+1=4-1+1=4，这是错误的，因为bca都不在窗口中了。所以当遇到重复字符时，要判断之前的字符位置k0与left的大小，如果k0<left，说明之前的字符已经在窗口之外了，这时不能更改left，仅当k0>=left时，左边界才向右移动一位。
			if k0 >= left {
				// 缩小左边窗口，向右边移动一位
				left = k0 + 1
			}
